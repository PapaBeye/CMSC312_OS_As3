...
#define POOLSIZE 25 //Size of buffer pool
...
struct Node
{
	...
	struct timespec stopwatch; //Time job was put into the queue
};
...
struct Buffer
{
	...
	double totalWaiting; //Total time spent waiting in the buffer, in seconds
...
void addBuffer(int payload, int processID, struct Buffer *buffer)
{
	...
	new.next = NULL;
	new.valid = 1;
	clock_gettime(CLOCK_MONOTONIC, &(new.stopwatch)); //new.stopwatch = (int) time(NULL);
...
void removeBuffer(int result[2], struct Buffer *buffer)
{
	...
	//Calculate time spent waiting on buffer, add to total
	clock_gettime(CLOCK_MONOTONIC, &current);
	buffer->totalWaiting = buffer->totalWaiting + (double)(current.tv_sec - buffer->head->stopwatch.tv_sec) + ((double)(current.tv_nsec - buffer->head->stopwatch.tv_nsec)) / 1000000000;
	//buffer->totalWaiting = buffer->totalWaiting + ((int) time(NULL) - buffer->head->stopwatch); 
	...
	return; //Return process ID that added job, and payload in result
}
...
//Returns the current size of the buffer, used in testing
