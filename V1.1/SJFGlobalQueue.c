...
//TODO:
// 1 - Test in shared memory using multiple processes *DONE*
// 2 - Test in shared memory using mix of threads and processes *DONE*
...
void initalizeBuffer(struct Buffer *buffer)
{
	...
	sem_init(&(buffer->full_sem), 1, POOLSIZE);
	sem_init(&(buffer->empty_sem), 1, 0);
	buffer->head = NULL;
